...
type InitPhase = "run";
const INIT_PHASES: readonly InitPhase[] = ["run"];

function initialServices(): ServiceRow<InitPhase>[] {
  return INIT_STEPS.map((name) => ({
    name,
    phases: { run: "pending" as PhaseState },
    status: null,
    error: null,
  }));
}

interface InitClusterUIProps {
  config: IxConfig;
  onDone: (result: { clusterIp: string; err?: Error }) => void;
}

const InitClusterUI: React.FC<InitClusterUIProps> = ({ config, onDone }) => {
  const [services, setServices] =
    useState<ServiceRow<InitPhase>[]>(initialServices);
  ...
  useEffect(() => {
    ...
    const setRow = (
      step: InitStep,
      next: Partial<ServiceRow<InitPhase>> & { runState?: PhaseState },
    ) => {
      setServices((rows) =>
        rows.map((row) =>
          row.name === step
            ? {
                ...row,
                phases:
                  next.runState != null ? { run: next.runState } : row.phases,
                status: next.status !== undefined ? next.status : row.status,
                error: next.error !== undefined ? next.error : row.error,
              }
            : row,
        ),
      );
    ...
    const run = async (
      ...
      setRow(step, { runState: "running" });
      try {
        await fn();
        setRow(step, { runState: "done" });
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        setRow(step, { runState: "failed", error: msg });
        throw err;
      }
    };
    ...
    const runAll = async (): Promise<void> => {
      ...
      // Step 6: TLS border for ingress-nginx
      ...
      // Step 8: NFR-003 namespace contract + identity Secret-deletion RBAC
      ...
      // Step 9: collect DNS info (FR-007-AC-5)
    ...
    runAll()
      .then(() => {
        ...
        const dnsTail = clusterIp
          ...
          : undefined;
        setFinalState({ status: "passed", tail: dnsTail });
        onDone({ clusterIp });
        setTimeout(exit, 0);
      })
      .catch((err: Error) => {
  ...
  return (
    <PhaseTable<InitPhase>
      header="ix · local · init-cluster"
      phases={INIT_PHASES}
      phaseLabels={{ run: "running" }}
      services={services}
      status={finalState?.status}
      tail={finalState?.tail}
      tailVariant={finalState?.tailVariant}
    />
  );
};
...
export async function runInitCluster(
  ...
  let captured: { err?: Error } = {};

  await render(
    <InitClusterUI
      config={config}
      onDone={(result) => {
        captured = { err: result.err };
      }}
    />,
  );
