...
1. **OS keyring** (FR-015), where ciphertext is held by the platform credential store (Keychain / libsecret / Credential Manager).
2. **age-encrypted blob** (FR-016) at `~/.config/ix/secrets.d/<plugin-id>.age`, encrypted with an X25519 identity at `~/.config/ix/secrets.key`.

This NFR applies to:

- All secrets declared via `IxPlugin.secretsSchema` and persisted by `SecretsService.set`.
- The persistence channels enumerated in NFR-003-AC-1 (the file-write APIs ix-cli code calls). Channels enumerated there are exhaustive for the static-check assertion.

This NFR does NOT govern:

- Environment variables in the user's shell — those are operator-controlled.
- In-memory values during a single `ix` process — memory hygiene is best-effort, not absolute (see FR-016 "Memory hygiene").
- Kubernetes Secret manifests rendered by `ensureGhcrCredsInNamespace` and applied via `kubectl` — those are in-flight only and not persisted by ix-cli (existing posture, preserved).
- Transitively-derived material that consumers compute from secret values *after* `SecretsService.get(...)` returns. Consumers SHOULD NOT write derivatives to disk; the static check in NFR-003-AC-1 catches the most common patterns but is not a complete proof for arbitrary downstream code. Plugin authors are responsible for not persisting derived material.
...
- A dedicated security test (`packages/core/tests/secrets-no-plaintext.test.ts`) implements NFR-003-AC-1, NFR-003-AC-2, and NFR-003-AC-5 as static greps and a round-trip leak scan.
- The integration test from NFR-003-AC-3 runs in CI on Linux (with and without dbus / Secret Service available) to exercise both backend paths.
