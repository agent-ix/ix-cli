...
- StR-008-AC-1: A generic CLI can depend on `@agent-ix/ix-cli-core`,
  declare its plugin set in `oclif.plugins`, and ship without depending on
  any IX service.
- StR-008-AC-2: An IX-connected CLI uses the same library plus IX service
  client plugins; no separate runtime exists.
- StR-008-AC-3: The main `ix` CLI is an oclif binary whose `oclif.plugins`
  lists the official Agent IX plugin packages.
- StR-008-AC-4: Plugin config, secrets, and env bindings are exposed
  through the `ixSchema` named export convention and registered by the
  host's `init` hook.
- StR-008-AC-5: Per-command capability requirements are declared as a
...
- A bespoke plugin registry, distribution object, manifest loader, or
  ...
  been superseded — see `spec/runtime-plugin-platform-plan.md`.
