...
**all** outstanding operator-scoped kubeconfigs derived from [FR-044](./FR-044-auth-kubeconfig-issue.md).
...
every scoped kubeconfig shares the same SA token (auth/[FR-008](./FR-008-ix-core-tag-convention.md) Operator
Privilege Lifecycle, Recovery section).

Optionally, with `--reissue --output <path>`, the command immediately
re-runs the [FR-044](./FR-044-auth-kubeconfig-issue.md) emission path against the freshly-recreated Secret so
...
|---|---|---|---|
| `--reissue` | no | `false` | After rotation completes, emit a fresh scoped kubeconfig at `--output` using the [FR-044](./FR-044-auth-kubeconfig-issue.md) path. Requires `--output`. |
| `--output <path>` | conditional | — | Required when `--reissue` is set; same semantics as [FR-044](./FR-044-auth-kubeconfig-issue.md)'s `--output`. |
...
4. **(Optional) Reissue.** If `--reissue` was supplied, invoke the
   [FR-044](./FR-044-auth-kubeconfig-issue.md) emission path with `--output` and (implicitly) `--force` so the
...
other output stream — same invariant as [FR-044-CON-3](./FR-044-auth-kubeconfig-issue.md).
...
| `reissue_failed` | `--reissue` was set and the [FR-044](./FR-044-auth-kubeconfig-issue.md) emission step failed | Propagates the [FR-044](./FR-044-auth-kubeconfig-issue.md) error envelope; the rotation itself has already succeeded at this point, so the caller MUST treat all outstanding scoped kubeconfigs as invalid even on this error. |

## Constraints

- **FR-045-CON-1**: The command SHALL require the same `kubectl get` on
  `system/ix-cli-admin-token` that [FR-044](./FR-044-auth-kubeconfig-issue.md) requires, plus `kubectl delete`
...
- **FR-045-CON-4**: The command SHALL NOT make any HTTP, HTTPS,
  ...
  permitted (matches [FR-044-CON-1](./FR-044-auth-kubeconfig-issue.md)).
- **FR-045-CON-5**: The decoded SA token SHALL NOT appear in stdout,
  ...
  variables (matches [FR-044-CON-3](./FR-044-auth-kubeconfig-issue.md)).
...
- Upstream: identity/[FR-034](./FR-034-refresh-changed-output.md), [FR-044](./FR-044-auth-kubeconfig-issue.md) (issue path reused for `--reissue`),
  auth/[FR-008](./FR-008-ix-core-tag-convention.md) (Operator Privilege Lifecycle Recovery section names this
