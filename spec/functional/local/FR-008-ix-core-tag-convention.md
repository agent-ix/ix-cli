...
|----|----------|--------------|
| FR-008-AC-1 | A service with `tags: ["ix-core"]` is included in the default deploy set. | Test |
| FR-008-AC-2 | A service without `ix-core` in its tags is excluded from the default deploy set unless listed in `extraApps`. | Test |
| FR-008-AC-3 | Tag matching is case-sensitive exact-string equality. | Test |
| FR-008-AC-4 | Multiple tags on a service — the service is included if any tag matches a `defaultTags` entry. | Test |
